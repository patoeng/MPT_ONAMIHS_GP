...
namespace TPM
{
    public partial class YCheckList : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                O = (Request.QueryString["o"] != null) && (Request.QueryString["o"] != "");
            }
            ...
            if ((session.IsPublic))
            {              
                Response.Redirect(session.Redirection);  
            }
            if (!IsPostBack)
            {
        ...
        {
            var sqlparams = new List<SqlParameter> {new SqlParameter("@LCheckListId", LchecklistId)};
            var ds = SqlHelper.ExecuteDataset(Functions.TPMDBConnection(), CommandType.StoredProcedure, "usp_getCheckList", sqlparams.ToArray());
            const string tag = "b";
            TableRow tr;
            TableCell tc;
            var dt1 = ds.Tables[1];
            ...
            foreach (DataRow dr1 in dt1.Rows)
            {
                ...
                Checklisttypeid = (int)dr1["Checklist_Type_id"];
              ...
              if (file.Exists)
                ...
                tr = new TableRow();
                tc = new TableCell();
                newControl = new HtmlGenericControl(tag) {InnerText = Checklisttypeid == 1 ? "Date" : "Month"};
                tc.Controls.Add(newControl);
                tr.Cells.Add(tc);
                ...
                if (Checklisttypeid == 1) {
                    var date = (DateTime)dr1["scheduled_date"];
                    newControl.InnerText = date.ToString("d MMM yyyy");
                }
                else
                {
                    newControl.InnerText = TPMHelper.Namabulan()[(int)dr1["month"]][1];
                }
                tc.Controls.Add(newControl);
                tr.Cells.Add(tc);
                TblAtasKiri.Rows.Add(tr);
                ...
                */
                ...
                if (Checklisttypeid == 1)
                {
                    tr = new TableRow();
                    tc = new TableCell();
                    newControl = new HtmlGenericControl(tag) {InnerText = "Machine Running"};
                    tc.Controls.Add(newControl);
                    tr.Cells.Add(tc);

                    running = dr1["IsRunning"].ToString().ToUpper();
                    tc = new TableCell();
                    newControl = new HtmlGenericControl(tag)
                        {
                            InnerText = dr1["IsRunning"].ToString().ToUpper() == "TRUE" ? "YES" : "NO"
                        };
                    newControl.Attributes.Add("class", "mesin_run_editable");
                    newControl.Attributes.Add("id", "runn_" + dr1["id"]);
                    tc.Controls.Add(newControl);
                    tr.Cells.Add(tc);
                    TblAtasKanan.Rows.Add(tr);
                }
            ...
            {
                checkedBy = dr0["Check_By_Operator"].ToString();
                dtChecked = dr0["Checked_Date"].ToString();
                checklistid = dr0["id"].ToString();
                result_asli = dr0["result"].ToString();
            ...
            {
                var thc = new TableHeaderCell {Text = dt2.Columns[iii].ColumnName};
                tr.Cells.Add(thc);
            }
            ...
            if (Checklisttypeid == 2)
            {
                foreach (DataRow dr2 in dt2.Rows)
                {
                    ...
                    if ((ds.Tables[0].Rows[0]["confirmed"].ToString().ToUpper() != "TRUE") && (dr2["methode"].ToString() != "") && ((checkedBy == string.Empty) || (result == "FAIL")) && showbutton ) { tc.CssClass = "editable"; }
                    tc.Text = dr2[iii].ToString() == ""
                                   ? ""
                                   : ChecklistStatus.GetStatus(Convert.ToInt32(dr2[iii].ToString()));
                    tr.Cells.Add(tc);
                    TblListItems.Rows.Add(tr);
                    iii++;
                    tc = new TableCell
            ...
            else
            {
                foreach (DataRow dr2 in dt2.Rows)
                {
                    ...
                    if ((ds.Tables[0].Rows[0]["confirmed"].ToString().ToUpper() != "TRUE") && (M == "") && ((checkedBy == string.Empty) || (result == "FAIL")) && showbutton && (running == "TRUE")) { tc.CssClass = "editable"; }
                    tc.Text = dr2[iii].ToString() == ""
                                  ? ""
                                  : ChecklistStatus.GetStatus(Convert.ToInt32(dr2[iii].ToString()));
                    tr.Cells.Add(tc);
            ...
            var imagewidth = jumlahimage > 1 ? 300 : 400;
            int o = 0;
            tr = new TableRow();
...
//img.Width = imagewidth;
  //              img.Height = (int)height;
                ...
                img.Style.Add("height", height.ToString(CultureInfo.InvariantCulture) + "px");
                tc.Controls.Add(img);
                tr.Cells.Add(tc);
                if (o < colnumber - 1)
                {
            ...
            tr = new TableRow();
            tc = new TableCell();
            var lb2 = new Label {Text = "Checked By"};
            tc.Controls.Add(lb2);
            ...
            if (checkedBy == string.Empty || result_asli==string.Empty)
            {
                ...
                lb.Text = "";
                lb.Attributes.Add("id", "user_"+LchecklistId);
