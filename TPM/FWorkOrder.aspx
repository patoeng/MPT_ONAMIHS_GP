...
<asp:Content ID="Content2" ContentPlaceHolderID="BodyContentHolder" runat="server">
     <div class="row-fluid">
        ...
        </div><div class="span6 text-right">Form No :xx.xx.xx</div>
    </div>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="FooterScriptsHolder" runat="server">
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#ddlSC').change(function () {
                if ($('#ddlSC option:selected').val() == "1") {
                    $('#ddlNC option').prop('selected', false)
                        .filter('[value="NC"]')
                        .prop('selected', true);
                    $('#ddlNC').prop('disabled', 'disabled');
                } else {
                    $("#ddlNC").removeAttr("disabled");
                }
            });
            ...
            $('#form1').validate({
                submitHandler: function () {
                    $.ajax({
                        ...
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        data: JSON.stringify({ deptid: $('#ddlDepartment option:selected').val(), assetid: $('#ddlAsset option:selected').val(), reason: $('#txtReason').val(), nc: $('#ddlNC').val(),sf:$('#ddlSC').val() }, null, 2),
                        success: function(json) {
